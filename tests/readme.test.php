<?php
/****************************************************************************** */
/** {{{*fheader
    {*file                  accountregister.php *}
    {*purpose               Script that tests a Simple Account Register *}
    {*author                {PYB} *}
    {*company               Lato Sensu Management[br]
                            Rue Bois des Mazuis, 47[br]
                            5070 Vitrival[br]
                            Belgium[br]
                            [url]http://www.trql.fm[/url][br]
                            Vae Soli! : [url]http://www.vaesoli.org[/url] *}
    {*cdate                 18-02-21 11:11:01 *}
    {*mdate                 auto *}
    {*license               All rights reserved to Lato Sensu Management
                            for all countries. All Intellectual Property
                            belongs to Patrick Boens. *}
    {*UTF-8                 Quel bel été *}

    ------------------------------------------------------------------------------
    Changes History:
    ------------------------------------------------------------------------------

    {*chist
        {*mdate 18-02-21 11:11:01 *}
        {*v 2.0.0001 *}
        {*desc              1)   Original creation
        *}
    *}

    *}}} */

    /** {{*accountingregister()=

        Test of a simple bank account

        {*params
        *}

        {*return
            [c]void[/c]
        *}

        {*author {PYB} *}
        {*cdate 18-02-21 11:11:01 *}

        {*example
            /tests/accountingregister.php
        *}

        *}}
     */
/****************************************************************************** */
use \trql\vaesoli\Vaesoli       as v;
use \trql\readme\Readme         as Readme;

if ( ! defined( 'VAESOLI_CLASS_VERSION' ) )
    require_once( 'd:/websites/snippet-center/trql.vaesoli.class.php' );

if ( ! defined( 'README_CLASS_VERSION' ) )
    require_once( 'd:/websites/snippet-center/trql.readme.class.php' );

$o = new Readme();

end:
var_dump( $o->lastInfo );

if ( $o->isDocumentationOutdated() )
{
    $o->document();
    var_dump( "Documentation updated" );
}

echo __LINE__," ... end of the script\n";
?>
